...
export function PhenomenaDirectory(){
 ...
 return <div>
  ...
  <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{list.map(p=><article key={p.slug} className="group overflow-hidden rounded-2xl border bg-card shadow-sm transition hover:-translate-y-1 hover:shadow-lg"><PhenomenonArt slug={p.slug} title={p.title} className="w-full"/><div className="p-5"><div className="mb-3 flex items-center justify-between gap-2"><Badge variant="outline">#{p.id}</Badge><span className="text-xs text-muted-foreground">{p.readingTime} min · {p.difficulty}</span></div><h2 className="text-xl font-bold tracking-tight">{p.title}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{p.summary}</p><p className="mt-3 text-xs font-medium text-primary">{p.category}</p><Button asChild className="mt-5 w-full"><Link href={`/phenomena/${p.slug}`}>Explore phenomenon <ArrowRight/></Link></Button></div></article>)}</div>
 </div>
}
