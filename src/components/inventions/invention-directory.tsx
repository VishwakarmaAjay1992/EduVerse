...
export function InventionDirectory({ inventions }: { inventions: Invention[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return inventions.filter((item) => {
      const matchesQuery =
        !normalized ||
        [item.title, item.summary, item.category, ...item.keywords]
          .join(" ")
          .toLowerCase()
          .includes(normalized);
      return (
  ...
  return (
    <section aria-labelledby="directory-heading">
      <h2 id="directory-heading" className="text-2xl font-bold tracking-tight">
        Browse inventions
      </h2>
      ...
      <p className="mt-4 text-sm text-muted-foreground" aria-live="polite">
        {filtered.length} invention{filtered.length === 1 ? "" : "s"}
      </p>
      <div className="mt-5 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((item) => (
          <InventionCard key={item.slug} invention={item} />
        ))}
      </div>
    </section>
  );
